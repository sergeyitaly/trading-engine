...
from ccxt.base.errors import InsufficientFunds, NetworkError, RequestTimeout, ExchangeError, OrderNotFound
...
class TPOrderConfig(BaseModel):
    price_percent: float = Field(..., gt=0, description="TP price percentage from entry")
    quantity_percent: float = Field(..., gt=0, le=100, description="Percentage of position to close")
...
class FastExchange:
    ...
    def __init__(self, exchange: ccxt.Exchange, max_retries: int = 3, timeout: int = 10):
    ...
    async def safe_request(self, method_name, *args, **kwargs):
        ...
        if not hasattr(self.exchange, method_name):
            raise AttributeError(f"'{self.exchange.name}' object has no attribute '{method_name}'")
            ...
            for retry in range(self.max_retries):
                try:
                    ...
                    if retry == self.max_retries - 1:
                        logger.error(f"Final timeout/network error in {method_name} after {self.max_retries} retries: {e}")
                        raise
                    logger.warning(f"Timeout/network error in {method_name}, retry {retry + 1}/{self.max_retries}: {e}")
...
class PositionManager:
    ...
    async def load_market_info(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not set leverage (proceeding anyway): {leverage_error}")
            ...
            for symbol_var in symbol_variations:
                ...
                for market_symbol, market_info in self.exchange.exchange.markets.items():
                    if market_symbol.lower() == config_symbol_lower:
                        self.market_info = market_info
                        logger.info(f"Found market using case-insensitive match: {market_symbol}")
                        break
                else:
                    raise ValueError(f"Market {self.config.symbol} not found. Available symbols: {len(all_symbols)} total")
            
            logger.info(f"Market info loaded: {self.market_info['symbol']}")
            logger.info(f"Market precision: amount={self.market_info['precision']['amount']}, price={self.market_info['precision']['price']}")
    ...
    async def calculate_contract_size(self, amount_usdt: float, price: float = None) -> float:
        """Calculate contract size based on amount in USDT"""
        if not price:
            ticker = await self.exchange.safe_request('fetch_ticker', self.config.symbol)
            price = ticker['last']
        ...
        return max(contract_size, self.market_info['limits']['amount']['min'])
    ...
    async def verify_order_execution(self, order_id: str) -> bool:
        ...
        try:
            order_status = await self.exchange.safe_request('fetch_order', order_id, self.config.symbol)
            
            if order_status['status'] == OrderStatus.CLOSED and float(order_status['filled']) > 0:
                logger.info(f"Order {order_id} successfully filled: {order_status['filled']}")
                return True
            else:
                logger.warning(f"Order {order_id} not fully filled. Status: {order_status['status']}, Filled: {order_status['filled']}")
                return False
    ...
    async def update_position_info(self):
        ...
        try:
            positions = await self.exchange.safe_request('fetch_positions', [self.config.symbol])
            
            for position in positions:
                if (position['symbol'] == self.config.symbol and 
                    abs(float(position['contracts'])) > 0 and
                    position['side'] == self.config.side.value):
                    
                    self.position = position
                    self.average_entry_price = float(position['entryPrice'])
                    ...
                    logger.info(
                        ...
                        f"Leverage: {position['leverage'] if 'leverage' in position else 'N/A'}"
                    )
                    return True
    ...
    async def set_leverage(self):
        ...
        try:
            ...
            if not self.exchange.exchange.has.get('setLeverage', False):
                logger.info(f"Exchange doesn't support API leverage setting. Using default leverage.")
                return
                ...
                for position in positions:
                    if position['symbol'] == self.config.symbol:
                        ...
                        if current_leverage == self.config.leverage:
                            logger.info(f"Leverage already set to {self.config.leverage}x for {self.config.symbol}")
                            return
                        break
            except:
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.info(f"Leverage set to {self.config.leverage}x for {self.config.symbol}: {result}")
                    return
                except ExchangeError as e:
                    error_msg = str(e)
                    # Handle specific Bybit leverage errors
                    if "110043" in error_msg or "leverage not modified" in error_msg:
                        logger.info(f"Leverage already set to {self.config.leverage}x or cannot be modified at this time")
                        return
                    elif "110026" in error_msg or "insufficient margin" in error_msg:
                        logger.warning(f"Cannot set leverage due to insufficient margin or open positions")
                        return
                    elif attempt == 2:  # Final attempt
                        logger.error(f"Failed to set leverage after {attempt + 1} attempts: {e}")
                        raise
                    else:
                        logger.warning(f"Leverage setting failed (attempt {attempt + 1}), retrying: {e}")
    ...
    async def place_tp_orders(self):
        ...
        for tp_config in self.config.tp_orders:
            ...
            if self.config.side == PositionSide.LONG:
                tp_price = self.average_entry_price * (1 + tp_config.price_percent / 100)
            else:
                tp_price = self.average_entry_price * (1 - tp_config.price_percent / 100)
            ...
            try:
                ...
                logger.info(f"TP order placed: {order['id']} - {tp_size} contracts at ${tp_price}")
    ...
    async def cancel_tp_orders(self):
        ...
        tp_orders = [o for o in self.open_orders if o.get('reduceOnly') or 
                    (o.get('info') and 'reduce_only' in o.get('info', {}))]
        
        for order in tp_orders:
            try:
                await self.exchange.safe_request('cancel_order', order['id'], self.config.symbol)
                logger.info(f"Cancelled TP order: {order['id']}")
            except (OrderNotFound, ExchangeError) as e:
                logger.warning(f"TP order {order['id']} not found or already cancelled: {e}")
            except Exception as e:
    ...
    async def place_averaging_orders(self):
        ...
        for i, price in enumerate(prices):
            try:
                ...
                # Calculate contract size
                contract_size = await self.calculate_contract_size(amount_per_order, price)
                ...
                self.open_orders.append(order)
                logger.info(f"Averaging order {i+1}/{orders_count} placed: {order['id']} - {contract_size} contracts at ${price}")
    ...
    async def check_orders(self):
        ...
        try:
            # Get open orders
            open_orders = await self.exchange.safe_request('fetch_open_orders', self.config.symbol)
            ...
            for order in self.open_orders[:]:  # Create a copy for iteration
                ...
                current_order = next((o for o in open_orders if o['id'] == order_id), None)
                
                if not current_order:  # Order is no longer open
                    try:
                        order_history = await self.exchange.safe_request('fetch_order', order_id, self.config.symbol)
                        
                        if order_history['status'] == OrderStatus.CLOSED:  # Order was filled
                            logger.info(f"Order {order_id} filled: {order_history['filled']}")
                        ...
                        logger.warning(f"Order {order_id} not found, assuming cancelled")
                        orders_to_remove.append(order_id)
                    except Exception as e:
                        logger.error(f"Error checking order {order_id}: {e}")
            
            # Remove processed orders
            self.open_orders = [o for o in self.open_orders if o['id'] not in orders_to_remove]
    ...
    async def update_tp_orders(self):
        ...
        logger.info(f"Updating TP orders based on new average price: ${self.average_entry_price}")
        await self.place_tp_orders()
    ...
    async def monitor_position(self):
        ...
        while self._running:
            try:
                ...
                if check_counter % 3 == 0 and self.config.move_sl_to_breakeven and self.position:
    ...
    async def check_breakeven_sl(self):
        ...
        try:
            ...
            if self.config.side == PositionSide.LONG:
                ...
                profit_pct = (self.average_entry_price - current_price) / self.average_entry_price * 100
                # Move SL to breakeven if profit reaches trailing SL offset
                if profit_pct >= self.config.trailing_sl_offset_percent:
                    await self.update_stop_loss(self.average_entry_price)
    ...
    async def close_all_orders(self):
        ...
        try:
            orders = await self.exchange.safe_request('fetch_open_orders', self.config.symbol)
            for order in orders:
                try:
                    await self.exchange.safe_request('cancel_order', order['id'], self.config.symbol)
...
class TradingEngine:
    ...
    async def init_exchange(self):
        ...
        try:
            ...
            if "bybit" in account_type:
                ...
                if not api_key or not api_secret:
                    raise ValueError(f"Bybit {('Testnet' if testnet else 'Mainnet')} API credentials not found. Please configure in UI or environment variables")
                ...
                if "testnet" in account_type:
                    ...
                    if not api_key or not api_secret:
                        api_key = os.getenv("GATEIO_TESTNET_API_KEY") or os.getenv("GATE_TESTNET_API_KEY")
                        api_secret = os.getenv("GATEIO_TESTNET_API_SECRET") or os.getenv("GATE_TESTNET_API_SECRET")
                    ...
                    # Fall back to environment variables - try both naming conventions
                    if not api_key or not api_secret:
                        api_key = os.getenv("GATEIO_API_KEY") or os.getenv("GATE_API_KEY")
                        api_secret = os.getenv("GATEIO_API_SECRET") or os.getenv("GATE_API_SECRET")
                    
                    testnet = False  # Set testnet for Gate.io mainnet
                
                if not api_key or not api_secret:
                    raise ValueError(f"Gate.io {('Testnet' if testnet else 'Mainnet')} API credentials not found. Please configure in UI or environment variables")
                ...
                raise ValueError(f"Unsupported exchange specified in config: {self.config.account}")
            ...
            logger.info(f"Initialized {exchange_id} {'Testnet' if testnet else 'Mainnet'} exchange connection")
    ...
    async def test_connection(self):
        ...
        try:
            ...
            logger.info(f"Exchange connection test successful. Available balance: {balance.get('USDT', {}).get('free', 0):.2f} USDT")
            return True
        except ExchangeError as e:
            ...
            if "USER_NOT_FOUND" in error_msg and "transfer funds first" in error_msg:
                logger.warning("Gate.io connection successful but futures account not funded. Please deposit funds.")
