...
class WebSocketClient:
    ...
    def start(self):
        ...
        self.thread = threading.Thread(
            target=self._run_websocket, name="WebSocketThread"
        )
    ...
    async def _websocket_loop(self):
        ...
        while self.running and self.connection_retries < self.max_retries:
            try:
                print(
                    f"Attempting to connect to {self.uri} (attempt {self.connection_retries + 1}/{self.max_retries})"
                )
...
class TradingDashboard:
    ...
    def init_credential_storage(self):
        ...
        if "saved_credentials" not in st.session_state:
            ...
            if stored_creds and stored_creds.get("value"):
                try:
                    st.session_state.saved_credentials = json.loads(
                        stored_creds["value"]
                    )
    ...
    def load_credentials_from_storage(self):
        ...
        try:
            ...
            if stored_creds and stored_creds.get("value"):
                ...
                st.session_state.saved_credentials = (
                    st.session_state.api_credentials.copy()
                )
                return True
        except Exception as e:
            ...
            if e.response.status_code == 500:
                ...
                try:
                    ...
                    error_msg = error_detail.get(
                        "detail", error_detail.get("message", str(e))
                    )
                except:
                ...
                error_msg = (
                    f"HTTP error {e.response.status_code} from {endpoint}: {str(e)}"
                )
                return {"status": "error", "message": error_msg}
            ...
            for message in messages:
                try:
                    ...
                    if channel == "status":
                        ...
                        st.session_state.last_orders = data.get("data", {}).get(
                            "orders", []
                        )
                        st.session_state.last_update = time.time()
                    elif channel == "price":
    ...
    def get_status(self):
        ...
        current_time = time.time()
        if (
            hasattr(st.session_state, "last_status")
            and hasattr(st.session_state, "last_update")
            and current_time - st.session_state.last_update < 5
        ):  # 5 second cache
    ...
    def get_orders(self):
        ...
        current_time = time.time()
        if (
            hasattr(st.session_state, "last_orders")
            and hasattr(st.session_state, "last_update")
            and current_time - st.session_state.last_update < 5
        ):  # 5 second cache
    ...
    def validate_credentials(self):
        ...
        if not current_creds.get("api_key") or not current_creds.get("api_secret"):
            return (
                False,
                f"Please configure API credentials for {self.config['account']} in the sidebar",
            )
    ...
    def test_connection(self):
        ...
        symbol_for_api = self.config.get(
            "symbol_api", self.config["symbol"].replace("/", "")
        )
        ...
        if result:
            if result.get("status") == "success":
                ...
                if (
                    "USER_NOT_FOUND" in error_msg
                    and "transfer funds first" in error_msg
                ):
                    return {
                        "status": "error",
            ...
            return {
                "status": "error",
                "message": "❌ Failed to connect to trading engine. Please check if the server is running.",
            }
    ...
    def handle_order_error(self, error_message):
        ...
        if (
            "USER_NOT_FOUND" in error_message
            and "transfer funds first to create futures account" in error_message
        ):
        ...
        if (
            "API credentials not found" in error_message
            or "credentials not found" in error_message.lower()
        ):
        ...
        if (
            ...
            or "not enough for new order" in error_message
            or "ab not enough" in error_message
        ):
    ...
    def start_trading(self):
        ...
        payload = {"credentials": credentials_payload}
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            current_account = st.session_state.get(
                "current_account", self.config.get("account", "Bybit/Testnet")
            )
            ...
            if (
                "gate" in selected_account.lower()
                and "testnet" not in selected_account.lower()
            ):
                with st.expander("⚠️ Gate.io Funding Required", expanded=True):
                    ...
                    **Gate.io Futures Account Funding Required**
                    ...
                    if st.button("Open Gate.io Futures", key="open_gate_futures"):
                        st.markdown(
                            "[👉 Open Gate.io Futures](https://www.gate.io/futures_trade/USDTM/BTCUSDT)"
                        )
            ...
            with st.expander(
                ...
                expanded=not (
                    current_creds.get("api_key") and current_creds.get("api_secret")
                ),
            ):
                ...
                with st.form("api_credentials_form"):
                    ...
                    prefilled_key = (
                        current_creds.get("api_key", "")
                        if current_creds.get("api_key")
                        else ""
                    )
                    prefilled_secret = (
                        current_creds.get("api_secret", "")
                        if current_creds.get("api_secret")
                        else ""
                    )
            ...
            # Symbol input
            symbol_help = (
                "For Bybit: BTCUSDT (no slash). For Gate.io: BTC/USDT (with slash)"
            )
            ...
            if "bybit" in selected_account.lower():
                ...
                if "/" in self.config["symbol"]:
                    st.warning(
                        "Remove slash from symbol for Bybit. Using: "
            ...
            if "market_type" in self.config:
                market_type_index = (
                    market_types.index(self.config["market_type"])
                    if self.config["market_type"] in market_types
                    else 0
                )
            ...
            if "bybit" in self.config["account"].lower():
                ...
                if "category" in self.config:
                    category_index = (
                        categories.index(self.config["category"])
                        if self.config["category"] in categories
                        else 0
                    )
            ...
            self.config["move_sl_to_breakeven"] = st.checkbox(
                "Move SL to Breakeven", value=self.config["move_sl_to_breakeven"]
            )
            ...
            self.config["limit_orders"]["engine_deal_duration_minutes"] = (
                st.number_input(
            ...
            with col1:
                if st.button("Start Trading", type="primary"):
                    ...
                    if result and result.get("status") == "success":
                        ...
                        if not st.session_state.websocket_connected:
                            st.info(
                                "🔄 Auto-connecting WebSocket for real-time updates..."
                            )
                            self.init_websocket()
                            st.experimental_rerun()
                    elif result and result.get("status") == "warning":
                        st.warning(
                            result.get("message", "Trading started with warnings")
                        )
                        # Show specific guidance for common warning scenarios
                        st.info(
                            "💡 This usually means: 1) Insufficient balance 2) Leverage issues 3) API permissions. Check your exchange account."
                        )
                        ...
                        error_msg = (
                            result.get("message", "Unknown error")
                            if result
                            else "Failed to start trading"
                        )
                        st.error(f"Error: {error_msg}")
                        # Show additional help for common errors
                        if "Insufficient balance" in error_msg:
                            st.info(
                                "💡 Please: 1) Check your account balance 2) Reduce order size 3) Consider lowering leverage"
                            )
                        elif (
                            "Trading session active but encountering errors"
                            in error_msg
                        ):
                            st.info(
                                "💡 The trading engine started but encountered errors. Please: 1) Check account balance 2) Verify API permissions 3) Stop and restart trading"
                            )
                        elif "Trading is already in progress" in error_msg:
                            st.info(
                                "💡 Please stop the current trading session before starting a new one"
                            )
            ...
            if st.button("Test Exchange Connection", type="secondary"):
                ...
                if result and result.get("status") == "success":
                    ...
                    st.info(
                        "Check: 1) API key permissions 2) Testnet vs Mainnet 3) IP restrictions"
                    )
    ...
    def render_metrics(self, status):
        ...
        with col1:
            ...
            color = (
                ...
                if status_text == "ACTIVE"
                    ...
                    else "orange" if status_text == "WARNING" else "gray"
                )
            )
            st.markdown(
                f'<div style="color: {color}; font-weight: bold;">Status: {status_text}</div>',
                unsafe_allow_html=True,
            )
        ...
        with col2:
            ...
            account_display = self.config["account"].split("/")[
                0
            ]  # Show just "Bybit" or "Gate.io"
            st.markdown(
                f'<div style="color: {cred_color}; font-weight: bold;">{account_display}: {cred_icon}</div>',
                unsafe_allow_html=True,
            )
            ...
            if "position" in status and status["position"]:
                ...
                if side and side != "none" and entry_price is not None:
                    ...
                    color = (
                        "green"
                        if side == "long"
                        else "red" if side == "short" else "gray"
                    )
                    st.markdown(
                        f'<div style="color: {color}; font-weight: bold;">Position: {side_display}</div>',
                        unsafe_allow_html=True,
                    )
                elif side and side != "none" and entry_price is None:
                    # Position intended but not opened (insufficient balance)
                    st.markdown(
                        f'<div style="color: orange; font-weight: bold;">Position: {side.upper()} (FAILED)</div>',
                        unsafe_allow_html=True,
                    )
                else:
                    # No position intended
                    st.markdown(
                        '<div style="color: gray; font-weight: bold;">Position: NONE</div>',
                        unsafe_allow_html=True,
                    )
            else:
                st.markdown(
        ...
        if (
            credentials_valid  # ONLY SHOW IF CREDENTIALS ARE VALID
            and status.get("status") == "active"
            and "position" in status
            and status["position"]
            and status["position"].get("side") not in ["none", None]
            and status["position"].get("average_entry_price") is None
        ):
            ...
            with st.expander(
                "⚠️ POSITION OPENING FAILED - Click for details", expanded=False
            ):
                st.error(
                    """
        ...
        if hasattr(st.session_state, "last_update"):
            update_time = datetime.fromtimestamp(st.session_state.last_update).strftime(
                "%H:%M:%S"
            )
    ...
    def render_orders_table(self):
        ...
        if orders_data and "orders" in orders_data and orders_data["orders"]:
            ...
            def color_status(val):
                color = (
                    "green" if val == "closed" else "orange" if val == "open" else "red"
                )
        ...
        if "position" in status and status["position"]:
            ...
            if entry_price is not None:
                fig.add_hline(
                    ...
                    line_dash="dash",
                    line_color=(
                        "green" if status["position"].get("side") == "long" else "red"
                    ),
    ...
    def get_logs(self):
        ...
        log_files = ["trading_engine.log", "api_calls.log"]
    ...
    def filter_logs(
        self, logs, level_filter="all", search_text="", connection_filter="all"
    ):
        ...
        if level_filter != "all":
            filtered_logs = [
                log for log in filtered_logs if log["level"] == level_filter.upper()
            ]
        ...
        if search_text:
            ...
            filtered_logs = [
                ...
                if (
                    search_lower in log["message"].lower()
                    or search_lower in log["logger"].lower()
                    or search_lower in log["level"].lower()
                )
            ]
    ...
    def render_logs_tab(self):
        ...
        with col2:
            ...
            st.session_state.connection_filter = st.selectbox(
                "WebSocket Events",
                options=ws_filters,
                index=ws_filters.index(
                    st.session_state.get("connection_filter", "all")
                ),
        ...
        for log in filtered_logs[:200]:
            ...
            if any(
                ...
                "websocket" in log["message"].lower() or "ws " in log["message"].lower()
            ):
        ...
        if st.session_state.get("websocket_connected", False):
            ...
            if hasattr(st.session_state, "last_update"):
                st.caption(
                    f"Last refresh: {datetime.fromtimestamp(st.session_state.last_update).strftime('%H:%M:%S')}"
                )
    ...
    def calculate_avg_connection_duration(self, ws_events):
            ...
            if event["type"] == "connected":
                ...
                try:
                    start_time = datetime.strptime(
                        current_connection["start"], "%Y-%m-%d %H:%M:%S"
                    )
                    end_time = datetime.strptime(
                        current_connection["end"], "%Y-%m-%d %H:%M:%S"
                    )
        ...
        if connections:
            avg_duration = sum(conn["duration"] for conn in connections) / len(
                connections
            )
    ...
    def render_debug_section(self):
        ...
        with st.expander("🔧 Debug Information", expanded=False):
            ...
            st.code(
                f"Env Credentials Available: {list(env_creds.keys()) if env_creds else 'None'}"
            )
    ...
    def render(self):
            ...
            **Please make sure:**
            ...
            if st.button("Check Server Status"):
                ...
                if server_logs.get("logs"):
                    ...
                    st.info(
                        "Cannot retrieve server logs. Make sure the server is running."
                    )
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["Chart", "Orders", "Configuration", "Logs", "Debug"]
        )
