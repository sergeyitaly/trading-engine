...
class TestTradingEngine:
    ...
    async def mock_exchange(self):
        ...
        mock_exchange.market = AsyncMock(return_value=MOCK_MARKET_INFO)  # Make this async
        mock_exchange.fetch_ticker = AsyncMock(return_value=MOCK_TICKER)
        mock_exchange.create_market_order = AsyncMock(return_value={'id': 'order_1', 'status': 'closed'})
        mock_exchange.fetch_order = AsyncMock(return_value={'id': 'order_1', 'status': 'closed', 'filled': 0.01})
    ...
    async def trading_engine(self, mock_exchange):
        ...
        with patch('app.main.ccxt.bybit', return_value=mock_exchange):
            with patch('app.main.FastExchange') as mock_fast_exchange:
                ...
                mock_fast_exchange_instance.safe_request = AsyncMock(side_effect=safe_request_side_effect)
...
class TestPositionManager:
    ...
    async def position_manager(self, mock_exchange):
        ...
        with patch('app.main.FastExchange') as mock_fast_exchange:
            ...
            mock_fast_exchange_instance.safe_request = AsyncMock(
                side_effect=lambda method, *args, **kwargs: getattr(mock_exchange, method)(*args, **kwargs)
            )
            mock_fast_exchange.return_value = mock_fast_exchange_instance
    ...
    async def test_averaging_orders_placement(self, position_manager):
        ...
        assert success == True
        assert mock_exchange.create_limit_order.call_count == 3  # Three averaging orders
...
class TestConfigurationHandling:
    ...
    def test_config_validation(self):
        ...
        with pytest.raises(ValueError, match="Total TP quantity percentage cannot exceed 100%"):
            TradingConfig(
...
async def test_contract_size_calculation():
    ...
    manager = PositionManager(None, TradingConfig(
        account=MOCK_CONFIG["account"],
        symbol=MOCK_CONFIG["symbol"],
        side=MOCK_CONFIG["side"],
        market_order_amount=MOCK_CONFIG["market_order_amount"],
